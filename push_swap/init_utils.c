...
/*   Updated: 2025/01/22 20:35:10 by malaamir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void	cost_calcule_a(t_stack_node *a, t_stack_node *b)// this function calculates the cost of pushing a node from stack a to stack b
{
	int	len_a;
	int	len_b;
	...
	while (a)
	{
		...
		if (a->target_node->above_median)
			a->push_cost += a->target_node->index;// if the target node is above the median, we add the index of the target node
		else
			a->push_cost += len_b - (a->target_node->index);// if the target node is below the median, we add the length of stack b minus the index of the target node
		a = a->next;
	}
}
...
void	at_index(t_stack_node *stack)
{
	...
	while (stack)
	{
		stack->index = i;
		if (i <= median)
			stack->above_median = true; // if the index of the node is less than the median, we set the above_median flag to true
		else
			stack->above_median = false;// if the index of the node is greater than the median, we set the above_median flag to false
		stack = stack->next;
		++i;
	}
}
